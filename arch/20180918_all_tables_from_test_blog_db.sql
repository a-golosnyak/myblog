...
--
-- Хост: 127.0.0.1:3306
-- Время создания: Сен 18 2018 г., 17:54
...
INSERT INTO `comments` (`id`, `post_id`, `author_id`, `parent_comment_id`, `pub_date`, `body`) VALUES
(1, 55, 7, 0, '2018-09-17 14:26:59', 'Здравствуйте. Мне очень понравилась статья! IFwjYnwLhsxyuNzZxQYv'),
(2, 55, 1, 0, '2018-09-18 09:53:52', 'Комментарий номер 2.'),
(3, 55, 1, 4, '2018-09-18 09:57:35', 'In Master mode, SYNC terminal is an output, in Slave mode, SYNC terminal is an input for a clock input. TTL\r\nlogic levels with compliance to GVDD.'),
(4, 55, 1, 0, '2018-09-18 10:00:07', 'In Master mode, SYNC terminal is an output, in Slave mode, SYNC terminal is an input for a clock input. TTL\r\nlogic levels with compliance to GVDD.'),
(5, 55, 7, 4, '2018-09-18 10:01:58', 'Есть куча способов изменить стандартное оформление Input File, но к сожалению, многие из них оказываются слишком громоздкими и зачастую не кроссбраузерными.');
...
(56, 7, 5, '2018-09-17 10:51:37', 'Авторизация через социальные сети: Facebook.', 'В данное время социальные сети пользуются огромнейшей популярностью среди пользователей интернета. Поэтому иногда бывает удобно при создании регистрации или авторизации пользователей на сайте использовать данные из социальной сети Facebook, что значительно облегчит вход пользователя на сайт. В данном уроке мы с Вами поговорим о том, как создать механизм авторизации пользователя на сайте, используя ресурсы социальной сети Facebook.', '../images/posts/2018-09-17_105137_Avtor_377.jpeg', '<h3>Получение параметра code</h3>\n\n<p>Итак, первым делом создадим файл конфигурации config.php:</p>\n\n<p>&nbsp;</p>\n\n<p>&nbsp;</p>\n\n<table>\n	<tbody>\n		<tr>\n			<td>\n			<p>1</p>\n\n			<p>2</p>\n\n			<p>3</p>\n\n			<p>4</p>\n\n			<p>5</p>\n\n			<p>6</p>\n\n			<p>7</p>\n\n			<p>8</p>\n			</td>\n			<td>\n			<p>&lt;?</p>\n\n			<p>define(&quot;URL_AUTH&quot;,&quot;https://www.Facebook.com/dialog/oauth&quot;);</p>\n\n			<p>define(&quot;CLIENT_ID&quot;,&quot;Идентификатор приложения&quot;);</p>\n\n			<p>define(&quot;SECRET&quot;,&quot;Секретный ключ приложения&quot;);</p>\n\n			<p>define(&quot;REDIRECT&quot;,&quot;http://avtomirock.com.ua/fb/auth.php&quot;);</p>\n\n			<p>define(&quot;TOKEN&quot;,&quot;https://graph.Facebook.com/oauth/access_token&quot;);</p>\n\n			<p>define(&quot;GET_DATA&quot;,&quot;https://graph.Facebook.com/me&quot;);</p>\n\n			<p>?&gt;</p>\n			</td>\n		</tr>\n	</tbody>\n</table>\n\n<p>&nbsp;</p>\n\n<p>Константы:</p>\n\n<p>URL_AUTH &ndash; адрес страницы авторизации Facebook;</p>\n\n<p>CLIENT_ID &ndash; идентификатор приложения;</p>\n\n<p>SECRET &ndash; секретный ключ приложения;</p>\n\n<p>REDIRECT &ndash; путь на который будет оправлен параметр code, в нашем случае мы создадим файл auth.php, который будет обрабатывать все данные;</p>\n\n<p>TOKEN &ndash; адрес по которому нужно отправить запрос для получения access_token &ndash; ключ доступа к данным пользователя;</p>\n\n<p>GET_DATA &ndash; адрес по которому нужно отправить запрос, для получения данных пользователя.</p>\n\n<p>Далее создадим файл functions.php в котором, будем описывать функции необходимые для работы скрипта. Затем на нашем сайте создаем страницу авторизации, к примеру, вот такого содержания (файл index.php):</p>\n\n<p>&nbsp;</p>\n\n<p>&nbsp;</p>\n\n<table>\n	<tbody>\n		<tr>\n			<td>\n			<p>1</p>\n\n			<p>2</p>\n\n			<p>3</p>\n\n			<p>4</p>\n\n			<p>5</p>\n\n			<p>6</p>\n\n			<p>7</p>\n\n			<p>8</p>\n			</td>\n			<td>\n			<p>&lt;?</p>\n\n			<p>require &quot;config.php&quot;;</p>\n\n			<p>require &quot;functions.php&quot;;</p>\n\n			<p>&nbsp;</p>\n\n			<p>$path = URL_AUTH.&quot;?&quot;.&quot;client_id=&quot;.CLIENT_ID.&quot;&amp;redirect_uri=&quot;.urlencode(REDIRECT).&quot;&amp;response_type=code&quot;;</p>\n\n			<p>?&gt;</p>\n\n			<p>&nbsp;</p>\n\n			<p>&lt;a href=&quot;&lt;? echo $path;?&gt;&quot;&gt;Авторизация&lt;/a&gt;</p>\n			</td>\n		</tr>\n	</tbody>\n</table>\n\n<p>&nbsp;</p>\n\n<p>Авторизация через Facebook начинается с авторизации непосредственно на данном сервисе. Поэтому мы должны перенаправить пользователя для авторизации. При этом мы должны отправить через адресную строку (методом GET) следующие данные:</p>\n\n<p>client_id &ndash; идентификатор приложения;</p>\n\n<p>redirect_uri &ndash; путь на который придет параметр code, в случае успешной авторизации;</p>\n\n<p>response_type &ndash; тип ответа от сервера, в нашем случае параметр code.</p>\n\n<p>Поэтому с учетом этих данных формируем путь для ссылки &ndash; переменная $path. Страница выглядит следующим образом:</p>\n');
...
(7, 'adm1@mail.ru', '111', 'adm1', '2018-09-13 09:19:40'),
